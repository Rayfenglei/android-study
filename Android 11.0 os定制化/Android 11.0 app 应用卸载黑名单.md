> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [blog.csdn.net](https://blog.csdn.net/baidu_41666295/article/details/124870689)

### 1. 概述

在 11.0 在进行定制化开发中，最近由需求要求对于安装卸载 app 的功能做限制, 对于限制 app 卸载的功能也是常有的功能，就是常说的 app 卸载黑名单功能，而 app 的安装和卸载都是有 [pms](https://so.csdn.net/so/search?q=pms&spm=1001.2101.3001.7020) 进行管理的

### 2.app 应用卸载黑名单的核心功能代码
...
### 3.app 应用卸载黑名单的核心功能分析和功能实现

### 3.1 IPackageManager.aidl 增加卸载接口
...
+       void setUnInstallPackageBlackList(in List<String> packageNames);
...
而在 PMS 中 deletePackageX 负责卸载 app 功能，增加卸载黑名单功能就从这里入手  
接下来看下 PMS 的源码

### 3.2 PackageManagerService.java 关于卸载方法相关代码
...
deletePackageX() 负责在 PMS 中进行 app 的卸载，所以要在此处在添加黑名单列表

### 3.3 关于实现卸载 app 黑名单功能的实现
...
+       private List<String> unInstallblackpackageNames;

+           @Override

+    public void setInstallPackageBlackList( List<String> packageNames) {

+               this.unInstallblackpackageNames=packageNames;
...
+               return this.unInstallblackpackageNames;

+    }

```

### 2.deletePackageX 添加过滤 app 安装黑名单列表
...
```
    public int deletePackageX(String packageName, long versionCode, int userId, int deleteFlags) {
       ...
       +  if(isUninstallBlackListApp(packageName)){
        +     return PackageManager.DELETE_FAILED_INTERNAL_ERROR;
        + }
...
+    private boolean isUninstallBlackListApp(String packagename){

 

+               if(this.unInstallblackpackageNames ==null || this.unInstallblackpackageNames.size()==0){
...
+        Iterator<String> it = this.unInstallblackpackageNames.iterator();
