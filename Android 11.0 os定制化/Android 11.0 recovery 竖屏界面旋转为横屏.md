> 本文由 [简悦 SimpRead](http://ksria.com/simpread/) 转码， 原文地址 [blog.csdn.net](https://blog.csdn.net/baidu_41666295/article/details/124828181)

在 11.0 项目开发中，由于平板固定横屏显示，而如果 [recovery](https://so.csdn.net/so/search?q=recovery&spm=1001.2101.3001.7020) 界面竖屏显示就觉得怪怪的  
所以今天就来解决这个问题

Android 的 Recovery 中，利用 bootable\recovery 下的 minui 库作为基础，采用的是直接存取 [framebuffer](https://so.csdn.net/so/search?q=framebuffer&spm=1001.2101.3001.7020) 的方式，来完成 recovery 中所需的各种 UI 的绘制。

在 recovery 的源码中，跟 ui 显示相关的代码的大致结构为：

1 boottable/recovery 下的 ui.h , ui.cpp， screen_ui.h，screen_ui.cpp，以及 mi_screen_ui.h，**_screen_ui.cpp，

2 boottable/recovery/minui 下的 resources.cpp，graphics.cpp

其中 resources.cpp 提供的 api 主要用于图片资源的读取和加载，graphics.cpp 负责具体完成各类 ui 的绘制

既然 graphics.cpp 是负责各类 UI 的绘制 那么旋转方向的修改 就要从这里入手了  
下面来看 graphics.cpp 的源码
...
从上述源码中 可以看出 gr_get_width(const GRSurface* [surface](https://so.csdn.net/so/search?q=surface&spm=1001.2101.3001.7020)) 是获取屏幕的宽度  
gr_get_height(const GRSurface* surface) 获取屏幕的高度  
gr_init_font(const char* name, GRFont** dest) 获取字体大小  
gr_init() 主要是设置 RGB 和 屏幕旋转方向
...
在 gr_init() 中，设置 rotation_str 的值为 ROTATION_RIGHT
...
在 gr_init() 中，设置 rotation 的值为 GRRotation::RIGHT
